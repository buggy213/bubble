...
struct AppConfig {
  ...
  bool pathtracer_accumulate_bounces; // whether we accumulate light bounce or
                                      // only sample from the last bounce
...
public:
  ...
  void load_camera(std::string filename) { camera.load_settings(filename); }

  enum Mode { EDIT_MODE, RENDER_MODE, VISUALIZE_MODE };
...
private:
  ...
  */
  ...
  GLScene::SceneLight *init_light(Collada::LightInfo &light,
                                  const Matrix4x4 &transform);
  GLScene::SceneObject *init_sphere(Collada::SphereInfo &polymesh,
                                    const Matrix4x4 &transform);
  GLScene::SceneObject *init_polymesh(Collada::PolymeshInfo &polymesh,
                                      const Matrix4x4 &transform);
