...
bool DragDouble3(const char *label, const double *p_data, float v_speed) {
  return ImGui::DragScalarN(label, ImGuiDataType_Double, (void *)p_data, 3,
                            v_speed);
}

bool DragDouble(const char *label, const double *p_data, float v_speed) {
  return ImGui::DragScalar(label, ImGuiDataType_Double, (void *)p_data,
                           v_speed);
}

bool SliderDouble3(const char *label, const double *p_data, float min,
                   float max) {
  return ImGui::SliderScalarN(label, ImGuiDataType_Double, (void *)p_data, 3,
                              &min, &max, "%f");
}
...
void test_light(SceneObjects::SceneLight *l) {
  ...
  ImGui::Text("sample_L(p = {%f, %f, %f}, &wi, &distToLight, &pdf);", p[0],
              p[1], p[2]);
...
VisualDebugger::VisualDebugger(GLScene::Scene **parent_scene,
                               int *current_mode) {
